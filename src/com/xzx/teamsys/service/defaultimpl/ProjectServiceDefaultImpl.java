...
public class ProjectServiceDefaultImpl extends WebServiceDefaultImpl implements
		ProjectService
{
	...
	public ProjectServiceDefaultImpl(ProjectDAO projectDAO, GroupDAO groupDAO,
			ProjectUserLinkerDAO projectUserLinkerDAO,
	...
	public int deleteProject(int projectId)
	{
		try
		{
			...
			List<ProjectUserLinker> projectUserLinkers = projectUserLinkerDAO
					.getLinkerByProjectId(projectId,
							EnumSet.allOf(ContributorStatus.class));
			...
			ContributorStatus status)
	{
		...
		{
			List<ProjectUserLinker> projectUserLinkers = projectUserLinkerDAO
					.getLinkerByUserId(owner, status);
			for (ProjectUserLinker projectUserLinker : projectUserLinkers)
			{
				Project project = projectDAO.getProjectById(projectUserLinker
						.getProjectId());
				Group group = groupDAO.getGroupByUserIdAndProjectId(
						projectUserLinker.getUserId(),
						projectUserLinker.getProjectId());
				UserProjectInfo userProjectInfo;
				userProjectInfo = new UserProjectInfo(
						projectUserLinker.getUserId(), project, group, status);
			...
			for (ContributorStatus status : statuses)
			{
				...
				for (ProjectUserLinker projectUserLinker : projectUserLinkers)
				{
					Project project = projectDAO
							.getProjectById(projectUserLinker.getProjectId());
					Group group = groupDAO.getGroupByUserIdAndProjectId(
							projectUserLinker.getUserId(),
							projectUserLinker.getProjectId());
					UserProjectInfo userProjectInfo = new UserProjectInfo(
							projectUserLinker.getUserId(), project, group,
							status);
					userProjectInfos.add(userProjectInfo);
				}
			}
	...
	public int inviteUser(int userId, int projectId)
	{
		try
		{
			ProjectUserLinker projectUserLinker = projectUserLinkerDAO
					.getLinkerByUserIdAndProjectId(userId, projectId);
			if (projectUserLinker == null)
			{
				projectUserLinker = new ProjectUserLinker(userId, projectId,
						ContributorStatus.DEFAULT);
	...
	public int accept(int linkerId)
	{
		try
		{
			...
			groupDAO.addUserToGroup(projectUserLinker.getUserId(),
					project.getDefaultGroupId());
